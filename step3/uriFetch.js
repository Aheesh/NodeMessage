...
async function URI_Fetch(formdata,myCallback) 
{
    ...
    const f = `{"Sender Wallet Address" :"${z}", "Recipient Wallet Address" :"${x} ", "Message" :"${y}", "Timestamp" :"${dateTime}" }`;
    console.log(f);
...
async function myDisplayer(some) {
  ...
  console.log(` uriFetch.js ---> myDisplayer callback object : ${some}`);

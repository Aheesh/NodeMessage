...
const nftContract = await new web3.eth.Contract(abistring, contractAddress);
  ...
  const transactionReceipt = await nftContract.methods.registerArtwork(address,encryptedString).send({from:accounts[0]});
  ...
  tbl.style.border = '1px solid black'
    ...
    for (let i = 0; i < tokens; i++) {
        ...
        console.log("Token Index: ", token_index, "Value: " ,i,"Message URI:",decryptedPayload);
